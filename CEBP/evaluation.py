...
# evaluator without y_counterfacutal,m0,m1. only for cancer
class EvaluatorCancer(object):
    def __init__(self, y, t, y_cf=None, mu0=None, mu1=None):
        self.y = y
        self.t = t
        self.y_cf = y_cf
        ...
        if mu0 is not None and mu1 is not None:
            self.true_ite = mu1 - mu0
    def ate_mean_ite(self, ypred1, ypred0):
        pred_ite = np.zeros_like(ypred1)
        idx1, idx0 = np.where(self.t == 1), np.where(self.t == 0)
        ite1, ite0 = self.y[idx1] - ypred0[idx1], ypred1[idx0] - self.y[idx0]
        pred_ite[idx1] = ite1
        pred_ite[idx0] = ite0
        return np.mean(pred_ite)
    def ate_mean_pred(self, ypred1, ypred0):
        return np.mean(ypred1 - ypred0)
    ...
    def y_errors(self, y0, y1):
        ypred = (1 - self.t) * y0 + self.t * y1
        return self.y_errors_pcf(ypred)

    def y_errors_pcf(self, ypred):
        rmse_factual = np.sqrt(np.mean(np.square(ypred - self.y)))
        return rmse_factual

    def calc_stats(self, ypred1, ypred0):
        ite = self.ate_mean_ite(ypred1, ypred0)
        ate = self.ate_mean_pred(ypred1, ypred0)
        return ite, ate